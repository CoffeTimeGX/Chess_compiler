        ...
        if self.first_move:
                if self.col == 0:
                    if Board[row][col+3] == 0:
                        self.available_moves.append((row,col+3))
                if self.col == 7:
...
class King(Piece):
    ...
    def get_available_moves(self,row,col,Board):
        ...
        try: #Verification de la possibilité du roque
            if self.first_move:
                if Board[row][col+1] == 0 and Board[row][col+2] == 0 and Board[row][col+3].color == self.color and Board[row][col+3].type == "Rook" and Board[row][col+3].first_move:
                    self.available_moves.append((row, col+2))
                    self.castles_possible = True
                if Board[row][col-1] == 0 and Board[row][col-2] == 0 and Board[row][col-3] == 0 and Board[row][col-4].color == self.color and Board[row][col-4].type == "Rook" and Board[row][col-4].first_move:
                    self.available_moves.append((row, col-2))
                    self.castles_possible = True
