...
class newBoard():
    def __init__(self, Width, Height, Rows, Cols, Square, Win):
        ...
        self.Board = []
        self.create_Board()
        self.roque = [False, False, False, False]


    def promote(self,row,col,promotion,turn):
        if turn == Black:
            if promotion == "reine":
                self.Board[row][col] = Queen(self.Square, White_Queen,White,"Queen",row,col)
            elif promotion == "cavalier":
                self.Board[row][col] = Knight(self.Square, White_Knight, White,"Knight",row,col)
            elif promotion == "tour":
                self.Board[row][col] = Rook(self.Square, White_Rook,Black,"Rook",row,col)
            elif promotion == "fou":
